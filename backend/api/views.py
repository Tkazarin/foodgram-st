...
from api.serializers.recipes import CartSerializer, RecipeDetailSerializer, RecipeCreateSerializer, \
    FavoriteRecipeSerializer
from api.serializers.users import UserAvatarSerializer, SubscriptionDetailSerializer, \
    SubscriptionCreateSerializer, UserDetailSerializer, UserRegistrationSerializer
...
class UserActionsViewSet(AuthUserViewSet):
    ...
    def delete_avatar(self, user):
        if user.avatar:
            if hasattr(user.avatar, 'url_path') and user.avatar.name != "users/image.png":
